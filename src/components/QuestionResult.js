...
export default function QuestionResult({
  ...
  let optionBC = [
    <div
      id={questionObj.id + '-B'}
      className={
        optionColorHandler('B', givenAnswer, actualAnswer) + optionStyling
      }
    >
    ...
    <div
      id={questionObj.id + '-C'}
      className={
        optionColorHandler('C', givenAnswer, actualAnswer) + optionStyling
      }
    >
      {' C. ' + questionObj.options[2]}
    </div>,
  ];
  ...
  return (
    <div className="flex flex-row gap-2 my-5 bg-slate-100 rounded-2xl p-8 shadow-lg">
      <div>
        <p>{questionObj.id}.</p>
      </div>
      <div className="grow max-w-full">
        <p className="mb-3 text-lg" style={{ overflowWrap: 'break-word' }}>
          {questionObj.body}
        </p>
            ...
            <div className={isTwoColumn ? 'grid grid-cols-2' : ''}>
              <div>
                <div
                  id={questionObj.id + '-A'}
                  className={
                    optionColorHandler('A', givenAnswer, actualAnswer) +
                    optionStyling
                  }
                >
                  {' A. ' + questionObj.options[0]}
                </div>
                {optionBC[0]}
              </div>
              <div>
                ...
                <div
                  id={questionObj.id + '-D'}
                  className={
                    optionColorHandler('D', givenAnswer, actualAnswer) +
                    optionStyling
                  }
                >
                  {'D. ' + questionObj.options[3]}
                </div>
              </div>
            </div>
            {questionObj.options[4] == null ? (
              ...
              <div>
                <div
                  id={questionObj.id + '-E'}
                  className={
                    optionColorHandler('E', givenAnswer, actualAnswer) +
                    optionStyling
                  }
                >
                  {'E. ' + questionObj.options[4]}
                </div>
              </div>
            )}
          </div>
