...
} from '@/components/addQuestionContext';
...
export default function setQuestionPage({ questionMetaData }) {
  ...
  return (
    <div>
      ...
      <Button
        className="fixed top-16 right-16"
        variant="primary"
        onClick={(e) => {
          ...
          copied.push(
            <SetQuestion key={copied.length} index={copied.length} />
          );
      ...
      <Button
        className="fixed top-32 right-16"
        variant="success"
        onClick={async (e) => {
          for (let i = 0; i < questionData.length; i++) {
            ...
            if (
              questionData[i].options.length > 4 &&
              questionData[i].options[4] == ''
            ) {
